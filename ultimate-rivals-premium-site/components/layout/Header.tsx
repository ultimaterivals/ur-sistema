            ...
            {navLinks.map((link) => {
              ...
              return (
                <Link
                  ...
                  className={`relative px-4 py-2 font-display font-semibold text-sm uppercase tracking-wider transition-colors duration-200 rounded-sm cursor-pointer ${
                    isActive
                      ? "text-ur-white bg-white/5"
                      : "text-ur-sand hover:text-ur-white hover:bg-white/5"
                  }`}
                >
                  {link.label}
                  {isActive && (
                    <span className="absolute bottom-0 left-1/2 -translate-x-1/2 w-4 h-px bg-ur-gold" />
                  )}
                </Link>
              );
            })}
                  ...
                  return (
                    <li key={link.href}>
                      <Link
                        ...
                        onClick={() => setMobileOpen(false)}
                        aria-current={isActive ? "page" : undefined}
                        className={`block px-3 py-2.5 font-display font-semibold text-sm uppercase tracking-wide rounded-sm transition-all duration-150 cursor-pointer ${
                          isActive
                            ? "text-ur-white bg-white/8 border-l-2 border-ur-gold"
                            : "text-ur-sand hover:text-ur-white hover:bg-white/5"
                        }`}
                      >
                        {link.label}
